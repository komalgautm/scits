...
use DB, Auth;
use App\Models\EarningScheme;
use App\Models\Incentive;
use App\Models\ServiceUser;
use App\Models\ServiceUserEarningDaily;
use App\Models\ServiceUserEarningStar;
use App\Models\Notification;
use App\Models\ServiceUserEarningIncentive;
use App\Models\Calendar;
use App\Models\HomeLabel;
use App\Models\ServiceUserEarningTarget;
use App\Models\ServiceUserIncentiveSuspend;
use App\Models\EarningAreaPercentage;
use App\Models\ServiceUserEarningRemove;
use App\Models\DynamicFormBuilder;
use App\Models\EarningSchemeLabel;
...
class EarningSchemeController extends Controller
{
    ...
    //    echo 1;die;
        ...
        $data['patient'] = DB::table('service_user')->where('id',$service_user_id)->where('is_deleted','0')->first();
        $data['daily_score']   = DB::table('daily_record_score')->get();

        $form_pattern['su_mfc'] = '';
        $form_pattern['su_bmp'] = '';
        $form_pattern['su_rmp'] = '';
        $data['dynamic_forms'] = DynamicFormBuilder::select('id','title','location_ids')->where('home_id',$su_home_id)->get();
        $data['service_users'] = ServiceUser::where('home_id',$su_home_id)->get();
        ...
        $data['earning_scheme'] = EarningScheme::where('home_id', $home_id)
                                        ->where('status', '1')
                                        ->orderBy('id','desc')
                                        ->get();
        // Ram comment this code 13/12/2023
        ...
        $data['total_stars'] = (int)$total_stars;
        ...
        for($i = 6; $i >= 0; $i--){
            ...
            $earn_daily = ServiceUserEarningDaily::where('service_user_id',$service_user_id)
                            ...
                            ->first();
            // echo"<pre>";print_r($earn_daily);die;
            if(isset($earn_daily) && $earn_daily !=''){
        ...
        $data['earn_history']  = EarningScheme::earningHistory($service_user_id);

        $data['notifications'] = Notification::getSuNotification($service_user_id,'','',4,$home_id);

        //su booked incentives to be shown in r.h.s of calendar
        $data['booked_incentives'] = EarningScheme::bookedIncentives($service_user_id);
        ...
        $data['earning_target'] = ServiceUserEarningTarget::getEarningTarget($service_user_id);
        
        $data['labels']  = HomeLabel::getLabels();
        $earning_scheme_label_ids = ServiceUser::where('id',$service_user_id)->where('home_id',$home_id)->value('earning_scheme_label_id');
        // echo"<pre>";print_r($earning_scheme_label_ids);die;
        // if(!empty($earning_scheme_label_ids)){
        if(isset($earning_scheme_label_ids) && $earning_scheme_label_ids !=''){
            $earning_scheme_label_ids = explode(',', $earning_scheme_label_ids);
            $data['earning_scheme_labels'] = EarningSchemeLabel::where('home_id',$home_id)
                                                        ->where('deleted_at',null)
                                                        ->whereIn('id',$earning_scheme_label_ids)
                                                        ->get();
                                                        // ->toArray(); 
        }else{
            $data['earning_scheme_labels']=array();
        }
        //echo "<pre>"; print_r($earning_scheme_label_ids);
        //die;
        $data['record_score'] = EarningScheme::getRecordsScore($service_user_id);
        $data['earn_area_percent'] = EarningAreaPercentage::select('daily_record','education_record','living_skill','mfc')->first();
        $data['guide_tag'] = 'earning';
        $data['service_user_id']=$service_user_id;
        $data['week_graph']=$week_graph;
        $data['form_pattern']=$form_pattern;
        // echo '<pre>'; print_r($earning_scheme_labels); die;
        return view('frontEnd.serviceUserManagement.earning_scheme',$data);
    }
