...
use DB,Auth;
use App\Models\ServiceUserPlacementPlan;
...
class PlacementPlanController extends Controller
{
    ...
    public function index(Request $request, $service_user_id = null) {
        ...
        $data['completed_targets'] = DB::table('su_placement_plan')
                                ->where('service_user_id',$service_user_id)
                                ->where('status','1')
                                ->paginate($this->completed);
        
        $data['active_targets'] = DB::table('su_placement_plan')
                                ->where('service_user_id',$service_user_id)
        ...
        $data['pending_targets'] = DB::table('su_placement_plan')
                                ->where('service_user_id',$service_user_id)
                                ->whereDate('date', '<=', $today)
                                ->where('status','0')
                                ->paginate(100);

        $data['service_user'] = ServiceUser::select('id', 'name')
        ...
        $form_pattern['placement_plan'] = '';
        $data['placement_plan_label'] = HomeLabel::getLabelbyTag('placement_plan');
        $data['guide_tag'] = 'placement';
        $data['service_user_id']=$service_user_id;
        return view('frontEnd.serviceUserManagement.placement_plan',$data);
    }
