...
use App\Models\StaffAnnualLeave;
use App\Models\StaffSickLeave;
use App\Models\StaffTaskAllocation;
...
class CalendarController1 extends Controller
{
    public function index(Request $request)
    {
        ...
        if (isset($user_type) && $user_type !='' &&  isset($user_id) && $user_id !='') {
            ...
            foreach ($service_users as $su_key => $service_user) {
                ...
                $health_records = ServiceUserHealthRecord::select('su_health_record.id as health_record_id', 'su_health_record.title')
                    ...
                    ->get()
                    ->toArray();
                // echo"<pre>";print_r(count($health_records));die;
                ...
                $education_records = ServiceUserEducationRecord::select('su_education_record.id as su_education_record_id', 'su_education_record.education_record_id', 'er.description')
                    ...
                    ->get()
                    ->toArray();
                // echo "<pre>"; print_r(count($education_records)); die;
                foreach ($education_records as $key => $education_record) {
                    //check if this living skill is booked in calendar
                    $event_id   = $education_record['su_education_record_id'];
                    $event_type = '10';
        ...
        $data['appointment_plans']=$appointment_plans;
        $data['system_calendar']=$system_calendar;
        $data['staff']=$staff;
        $data['log']=$log;
        $data['labels']=$labels;
        $data['guide_tag']=$guide_tag;
        $data['staff_members']=$staff_members;
        $data['user_type']=$user_type;
        $data['members']=$members;
        $data['ml_record']=$ml_record;
        $data['selected_user_id']=$selected_user_id;
        // echo"<pre>";print_r(count($data['log']['log_book']));die;
        
        return view('frontEnd.systemManagement.calendar',$data);
    }
