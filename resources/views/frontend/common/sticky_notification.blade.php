...
<!-- danger notifications -->
    <?php 
        $notifications = App\Models\Notification::getStickyNotifications();
...
<script>
    <?php  
        $alert_dynamic_form = App\Models\DynamicForm::alertDynamicForm();
...
<script type="text/javascript">
    ...
    foreach ($notifications as $key => $value) {
        ...
        $su_name         = App\Models\ServiceUser::where('id',$value['service_user_id'])->value('name'); 
        ...
        if($event_type_id == '14'){ //In danger
            ...
            if($event_action == 'ADD'){
                ...
                $su_loc_notif = App\Models\ServiceUserLocationNotification::select('id','location_name','location_type','old_location_type')
                                    ->where('id',$event_id)
                                    ->first();
...
<script>
    <?php  
        $su_risks_notifiy = App\Models\ServiceUserRisk::stickyNotifications();
