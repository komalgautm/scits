...
<form method="post" class="form-signin" action="" method="post" id="login_form" autocomplete="off">
    @csrf
    ...
    <div class="login-wrap">
        ...
        <div class="c-btn-group">
            <button class="btn btn-lg btn-login btn-block inp-hide sub_btn" type="button" id="submit">Sign in</button>
        </div>
</form>
<!-- <div> -->
...
<!-- Forgot Password Modal -->
<?php $all_companies = App\Models\Admin::select('id', 'company')
    ->where('access_type', 'O')
    ->where('is_deleted', '0')
    ->get()
    ->toArray();
    ...
    $('#submit').click(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                console.log(data);
                if ($.trim(data) == "logged_error") {
                    $('.popup_error').show();
                    $('.popup_error_txt').text("You are already logged in from some other device.");
                    $(".popup_error").fadeOut(5000);
                } else if ($.trim(data) == "invalid") {
                    $('.popup_error').show();
                    $('.popup_error_txt').text("Incorrect email or password combination.");
                    $(".popup_error").fadeOut(5000);
                } else {
                    <?php Session()->put('logged', 1); ?>
